...
const DyForm:React.FC<DyFormProps> = ({htmlForm,onSubmit,onError,formId,setLoading})=> {
  ...
  const handleChange = (e) => {
      ...
      case InputTypeEnum.Number:
        ...
        break
			case InputTypeEnum.Checkbox:
				content = (
					<Checkbox name='input.name' value={formData[input.name]} onChange={handleChange}  >
						{input.label}
					</Checkbox>
	...
	return (
      ...
      {formDetails.inputs.filter(input => input.type !== 'hidden').map(input => (
        <Box pb='0.7rem'  key={input.id}>
