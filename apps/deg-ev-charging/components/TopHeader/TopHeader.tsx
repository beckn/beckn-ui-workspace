  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-2.5 sm:py-3 flex items-center justify-between gap-3">
        <Link
          href="/"
          className="flex items-center gap-2 no-underline text-[var(--ev-text)] min-h-[var(--ev-touch-min)] items-center -my-2"
        >
          ...
          <span className="font-semibold text-base sm:text-lg text-[var(--ev-text)] truncate">{title}</span>
        </Link>
        {showBack && (
          <button
            type="button"
            onClick={() => router.back()}
            className="min-h-[var(--ev-touch-min)] min-w-[var(--ev-touch-min)] flex items-center justify-center text-sm font-medium text-[var(--ev-text-muted)] hover:text-[var(--ev-primary)] active:opacity-80 transition -my-2"
          >
            ← Back
          </button>
        )}
