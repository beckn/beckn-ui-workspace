...
const CustomerReviews: React.FC<ReviewProps> = ({ name, rating, reviewHeading, review, foundHelpful }) => {
  const { t } = useLanguage()
  return (
    <Box fontFamily={'Poppins'} mt={'15px'} w={'100%'}>
      <Flex align={'center'} gap={'8px'}>
        ...
        <Flex flexDirection={'column'}>
          <Text fontSize={'15px'} fontWeight={400}>
            {name} | <span style={{ fontSize: '12px' }}> {t('verifiedPurchase')}</span>
          </Text>
          <Flex gap={'9px'}>
      ...
      <Box mt={'12px'}>
        ...
        <Text fontSize={'12px'} fontWeight={400} mb={'5px'}>
          {review}
        </Text>
        <Text fontSize={'11px'} fontWeight={400} mb={'20px'}>
          {foundHelpful} {t('foundHelpfulText')}
        </Text>
      </Box>
      <Divider h={'1px'} bgColor={'#BFBFBF'} w={'100%'} p={'-15px'} />
    </Box>
  )
}
