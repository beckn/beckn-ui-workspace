...
const AvgReviews: React.FC<AvgReviewProps> = ({ avgRating, numOfReviewers }) => {
  const { t } = useLanguage()
  return (
    <Box p={'15px'} fontFamily={'Poppins'} w={'100%'}>
      <Text fontSize={'17px'} fontWeight={600} align={'left'} mb={'5px'}>
        {t('customerReviews')}
      </Text>
      ...
      <Text fontSize={'12px'} fontWeight={400} mt={'10px'} mb={'20px'}>
        {numOfReviewers} {t('verifiedUsersText')}
      </Text>
      <Divider h={'1px'} bg={'#BFBFBF'} />
    </Box>
  )
}
