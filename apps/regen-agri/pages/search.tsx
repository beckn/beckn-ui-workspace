...
const Search = () => {
  ...
  useEffect(() => {
    if (localStorage && localStorage.getItem('coordinates')) {
      const parsedCoordinates = JSON.parse(localStorage.getItem('coordinates') as string)
        ...
        const searchPayloadWithSearchQuery = {
          ...
          message: {
            criteria: {
              ...
              categoryName: 'Agriwaste',
              searchString: searchTerm
            }
          }
        }

        fetchData(`${apiUrl}/client/v2/search`, 'POST', searchPayloadWithSearchQuery)
      }
      ...
      if (providerId) {
        fetchData(`${apiUrl}/client/v2/search`, 'POST', searchPayloadWithLocValues)
      }
    }
  ...
  useEffect(() => {
    if (data) {
      ...
      localStorage.setItem('searchItems', JSON.stringify({ [providerId]: allItems }))
      setItems(allItems)
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [data])
