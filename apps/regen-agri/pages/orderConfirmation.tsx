...
const OrderConfirmation = () => {
  ...
  useEffect(() => {
    if (initResponse && paymentType.trim().length) {
      ...
      const payLoadForConfirmRequest = getPayloadForConfirmRequest(initMetaDataPerBpp, transactionId, paymentType)
  ...
  useEffect(() => {
    if (!initResponse && localStorage && localStorage.getItem('initResult') && paymentType.trim().length) {
  ...
  if (
    ...
    (confirmRequest.data &&
      (confirmRequest.data[0].message.responses.length === 0 || confirmRequest.data[0].message.responses?.[0]?.error))
  ) {
