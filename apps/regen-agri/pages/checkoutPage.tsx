...
const CheckoutPage = () => {
  ...
  const [billingFormData, setBillingFormData] = useState<ShippingFormData>(formData)
  const [quoteResponse, setQuoteResponse] = useState<any>(null)
  const [isBillingAddressCheckboxChecked, setIsBillingAddressCheckboxChecked] = useState(true)
  ...
  const fetchInit = (
    ...
    return initRequest.fetchData(`${apiUrl}/client/v2/initialize_order`, 'POST', payLoadForInitRequest)
  }
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (localStorage.getItem('shippingAdress') && localStorage.getItem('billingAddress')) {
        const shippingFormDataFromStorage = JSON.parse(localStorage.getItem('shippingAdress') as string)
        const billingFormDataFromStorage = JSON.parse(localStorage.getItem('billingAddress') as string)
        fetchInit(cartItems, transactionId, shippingFormDataFromStorage, billingFormDataFromStorage, apiUrl as string)
      }
    }
  ...
  useEffect(() => {
    if (localStorage && localStorage.getItem('quoteResponse')) {
      const stringifiedQuoteResponse = localStorage.getItem('quoteResponse')
  ...
  return (
    <>
            ...
            return (
              <>
                <ItemDetails
                  ...
                  price={formatCurrency(parseFloat(item.price.value), item.price.currency)}
                />
              </>
            )
          })}
        ...
        <Box>
          ...
          >
            ...
            <AddBillingButton
              ...
              addBillingdetailsBtnText={!isBillingAddressCheckboxChecked ? t.changeText : ''}
              billingFormSubmitHandler={formSubmitHandlerForBillingForm}
            />
          ...
          <DetailsCard>
            ...
            >
              <Checkbox
                ...
                fontSize={'17px'}
                defaultChecked
                onChange={() => setIsBillingAddressCheckboxChecked(prevValue => !prevValue)}
              >
        ...
        <Box>
          ...
          <DetailsCard>
            <PaymentDetails
              qoute={
                initRequest.data
                  ? initRequest.data[0].message.catalogs.responses[0].message.order.quote
                  : quoteResponse[0].message.catalogs.order.quote
              }
            />
          </DetailsCard>
        </Box>
      ...
      <ButtonComp
        ...
        handleOnClick={() => {
          fetchInit(cartItems, transactionId, formData, billingFormData, apiUrl as string)
        }}
