...
import { getDataPerBpp, getPayloadForOrderHistoryPost } from '../utilities/orderDetails-utils'
...
const OrderDetails = () => {
  ...
  useEffect(() => {
    if (localStorage && localStorage.getItem('selectedOrderFromHistory')) {
      const parsedOrderFromHistory = JSON.parse(localStorage.getItem('selectedOrderFromHistory') as string)
      const { bppId, bppUri, orderId, transaction_id } = parsedOrderFromHistory
  ...
  useEffect(() => {
    if (statusRequest.data) {
      ...
      const ordersPayload = getPayloadForOrderHistoryPost(statusRequest.data as StatusResponseModel[])
      axios
  ...
  const handleCancelButton = () => {
    ...
    return cancelRequest.fetchData(`${apiUrl}/client/v2/cancel`, 'POST', cancelPayload)
  }
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            ...
            <Text>{formatTimestamp(orderFromStatusResponse.created_at)}</Text>
          </Flex>
      ...
      >
        <CardBody
          ...
          pb={'unset'}
        >
          <PaymentDetails qoute={orderFromStatusResponse.quote as QuoteModel} />
        </CardBody>
        <CardBody
          pb={'unset'}
          pt={'15px'}
          ...
          >
            ...
            >
              ...
              <Box fontSize={'12px'}>
                ...
                {paymentState !== 'PAID' && (
                  <Link
                    onClick={() => window.open(paymentLink, '_blank', 'popup')}
                    color={'#4D930D'}
