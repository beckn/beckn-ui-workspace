...
export const getSubTotalAndDeliveryChargesForOrder = (confirmData: ResponseModel[] | null) => {
  ...
  if (confirmData) {
    confirmData.forEach(data => {
      const deliveryAmount = parseFloat(data.message.responses[0].message.order.quote.breakup[1].price.listed_value)
      totalDeliveryCharge += parseFloat(deliveryAmount.toFixed(2))

      const subTotalAmount = parseFloat(data.message.responses[0].message.order.quote.breakup[0].price.listed_value)
