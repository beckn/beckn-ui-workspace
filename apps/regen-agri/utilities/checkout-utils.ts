...
export const getPayloadForInitRequest = (
  ...
  const { address, city, country, email, mobileNumber, name, zipCode, state } = billingFormData
  const {
  ...
  if (initData) {
    initData.forEach(data => {
      const deliveryAmount = parseFloat(
        data.message.catalogs.responses[0].message.order.quote.breakup[1].price.listed_value
      ).toFixed(2)
