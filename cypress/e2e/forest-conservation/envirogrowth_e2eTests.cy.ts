...
describe('End to End test cases for Envirogrowth app', () => {
  const deployed_url = 'https://envirogrowth-dev.becknprotocol.io/signIn'
  ...
  const product = 'Digital soil data map of India'
  const selectProductForCheckout = 'Digital soil data map of India'
  const filterByPriceProduct = 'Digital soil data map of India'
  const filterByRatingProduct = 'Archive of climate and weather data'
  const filterReset = 'Archive of climate and weather data'
  const item_quantity = 'X 1'
  const item_price = '₹90.00'
  const totalPayment = '₹90.00'
  ...
  context('Product Details', () => {
    ...
    it('should validate and select all checkbox from every section ', () => {
      ...
      cy.getByData(testIds.product_checkbox).eq(1).should('contain.text', 'Raster')

      cy.getByData(testIds.product_details_checkbox).eq(1).should('contain.text', 'Spatial Resolutions')
      cy.getByData(testIds.product_checkbox).eq(2).should('contain.text', '500 m').click()

      cy.getByData(testIds.product_details_checkbox).eq(2).should('contain.text', 'Temporal Coverage')
      cy.getByData(testIds.product_checkbox).eq(3).should('contain.text', '1 year')
      cy.getByData(testIds.product_checkbox).eq(4).should('contain.text', '5 year')
      cy.getByData(testIds.product_checkbox).eq(5).should('contain.text', '10 year').click()

      cy.getByData(testIds.product_details_checkbox).eq(3).should('contain.text', 'Data Formats')
      cy.getByData(testIds.product_checkbox).eq(6).should('contain.text', 'XML')
      cy.getByData(testIds.product_checkbox).eq(7).should('contain.text', 'CSV').click()
      ...
      cy.getByData(testIds.product_checkbox).eq(8).should('contain.text', 'Accept').click()
