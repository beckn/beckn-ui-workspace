...
import { statusResponse } from '../../../fixtures/forest-conservation/ENVIROGROWTH/orderHistory/statusResponse'
...
describe('OrderHistory Page Tests', () => {
  context('should Render Empty Order Page when there is no Data in order Response', () => {
    before(() => {
      ...
      cy.intercept('GET', '**/api/orders?filters[category][id][$eq]=13&sort=updatedAt:desc', {
        fixture: 'forest-conservation/ENVIROGROWTH/orderHistory/emptyOrderResponse.json'
      }).as('getEmptyOrders')
    })
  ...
  context('Should render Order History Page if there is Order response Data ', () => {
    before(() => {
      ...
      cy.getByData(testIds.orderHistory_text_click).click()
      cy.intercept('GET', '**/api/orders?filters[category][id][$eq]=13&sort=updatedAt:desc', {
        fixture: 'forest-conservation/ENVIROGROWTH/orderHistory/orderResponse.json'
      }).as('orderResponse')
    })
