...
describe('Search Page Tests', () => {
  ...
  context('When there are no search results', () => {
    ...
    beforeEach(() => {
      ...
      cy.performSearch(searchTerm, {
        fixture: 'Forest-conservation/EARTH-SUPPORT-INITIATIVE/searchPage/emptySearchResults.json'
      })
    })
  ...
  context('When there are search results', () => {
    ...
    it('should handle sort by price option in filter', () => {
      ...
      cy.getByData(testIds.searchpage_products).eq(0).should('contain.text', 'Crowdsourced images data')
    })
    ...
    it('should handle filter by rating option in filter', () => {
      ...
      cy.getByData(testIds.searchpage_applyFilter).click()
      cy.getByData(testIds.searchpage_products).eq(0).should('contain.text', 'On-field data of restoration project')
    })
