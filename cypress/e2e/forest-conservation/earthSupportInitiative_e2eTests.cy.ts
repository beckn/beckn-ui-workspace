...
describe('End to End test cases for Earth Support Initiative app', () => {
  const deployed_url = 'https://earth-support-initiative-dev.becknprotocol.io/signIn'
  ...
  const product = 'On-field data of restoration project'
  const selectProductForCheckout = 'On-field data of restoration project'
  const filterByPriceProduct = 'Crowdsourced images data'
  const filterByRatingProduct = 'On-field data of restoration project'
  const filterReset = 'On-field data of restoration project'
  const item_quantity = 'X 1'
  const item_price = '₹15.00'
  const totalPayment = '₹15.00'
  ...
  context('Product Details', () => {
    ...
    it('should validate and select all checkbox from every section ', () => {
      cy.getByData(testIds.product_details_checkbox).eq(0).should('contain.text', 'Temporal Coverage')
      cy.getByData(testIds.product_checkbox).eq(0).should('contain.text', '1 month').click()
      cy.getByData(testIds.product_checkbox).eq(1).should('contain.text', '3 month')
      cy.getByData(testIds.product_checkbox).eq(2).should('contain.text', '1 year')
      cy.getByData(testIds.product_checkbox).eq(3).should('contain.text', '5 year').click()

      cy.getByData(testIds.product_details_checkbox).eq(1).should('contain.text', 'Data Formats')
      cy.getByData(testIds.product_checkbox).eq(4).should('contain.text', 'XML')
      cy.getByData(testIds.product_checkbox).eq(5).should('contain.text', 'CSV').click()
      cy.getByData(testIds.product_checkbox).eq(6).should('contain.text', 'HDF').click()

      cy.getByData(testIds.product_details_checkbox).eq(2).should('contain.text', 'Data Sharing Modes')
      ...
      cy.getByData(testIds.product_radio_text).eq(2).should('contain.text', 'Rest API')
      cy.getByData(testIds.product_radio_text).eq(3).should('contain.text', 'Email').click()

      cy.getByData(testIds.product_details_checkbox).eq(3).should('contain.text', 'Subscription Duration')
      cy.getByData(testIds.product_radio_text).eq(4).should('contain.text', '3 month')
      cy.getByData(testIds.product_radio_text).eq(5).should('contain.text', '6 month')
