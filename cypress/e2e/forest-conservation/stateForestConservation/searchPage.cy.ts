  ...
  context('When there are no search results', () => {
    ...
    beforeEach(() => {
      ...
      cy.performSearch(searchTerm, {
        fixture: 'forest-conservation/StateForestConservation/searchPage/emptySearchResults.json'
      })
    })
  ...
  context('When there are search results', () => {
    ...
    it('should handle sort by price option in filter', () => {
      ...
      cy.getByData(testIds.searchpage_products)
        .eq(0)
        .should('contain.text', 'Aerial LiDAR survey on Vanapur-Girivat belt data')
    })
