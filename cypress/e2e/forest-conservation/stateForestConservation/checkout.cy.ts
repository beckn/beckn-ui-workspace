...
import { billingDetails } from '../../../fixtures/forest-conservation/StateForestConservation/checkoutPage/userDetails'
import { initResponse } from '../../../fixtures/forest-conservation/StateForestConservation/checkoutPage/initResponse'
...
describe('Checkout Page', () => {
  ...
  before(() => {
    ...
    cy.intercept('POST', '/select', {
      fixture: 'forest-conservation/StateForestConservation/cart/selectResult.json'
    }).as('selectCall')
    cy.wait('@selectCall')
  ...
  it('should display the item details', () => {
    ...
    cy.getByData(testIds.item_title).should('contain.text', 'Aerial LiDAR survey on Vanapur-Girivat belt data')
    cy.get('[data-test="item-description"]').should('contain.text', 'Open Data Platform')
    cy.getByData(testIds.item_quantity).should('contain.text', 1)
    cy.getByData(testIds.item_price).should('contain.text', '₹50.00')
  })
  ...
  it('should display the payment breakup details', () => {
    cy.getByData(testIds.checkoutpage_paymentDetails).within(() => {
      ...
      cy.getByData(testIds.item_price).eq(0).should('contain.text', '₹50.00')
    })
  })
