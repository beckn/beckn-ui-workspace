...
describe('End to End test cases for State Forest Department app', () => {
  const deployed_url = 'https://state-forest-department-dev.becknprotocol.io/signIn'
  ...
  const searchTerm = 'forest field data'
  const wrongSearchTerm = 'cake'
  const product = 'Soil hydrology data'
  const selectProductForCheckout = 'Soil hydrology data'
  const filterByPriceProduct = 'Soil Survey data'
  const filterByRatingProduct = 'Aerial LiDAR survey on Vanapur-Girivat belt data'
  const filterReset = 'Aerial LiDAR survey on Vanapur-Girivat belt data'
  const item_quantity = 'X 1'
  const item_price = '₹40.00'
  const totalPayment = '₹40.00'
  ...
  context('Product Details', () => {
    ...
    it('should validate and select all checkbox from every section ', () => {
      cy.getByData(testIds.product_details_checkbox).eq(0).should('contain.text', 'Spatial Representation Type')
      cy.getByData(testIds.product_checkbox).eq(0).should('contain.text', 'Raster').click()

      cy.getByData(testIds.product_details_checkbox).eq(1).should('contain.text', 'Spatial Coverage')
      cy.getByData(testIds.product_checkbox).eq(1).should('contain.text', 'Beanibazar')
      cy.getByData(testIds.product_checkbox).eq(2).should('contain.text', 'Gopalganj')
      cy.getByData(testIds.product_checkbox).eq(3).should('contain.text', 'Balaganj')
      cy.getByData(testIds.product_checkbox).eq(4).should('contain.text', 'Biswanath')
      cy.getByData(testIds.product_checkbox).eq(5).should('contain.text', 'Kanaigath').click()

      cy.getByData(testIds.product_details_checkbox).eq(2).should('contain.text', 'Spatial Resolutions')
      cy.getByData(testIds.product_checkbox).eq(6).should('contain.text', '1 km')
      cy.getByData(testIds.product_checkbox).eq(7).should('contain.text', '5 km')
      cy.getByData(testIds.product_checkbox).eq(8).should('contain.text', '10 km').click()

      cy.getByData(testIds.product_details_checkbox).eq(3).should('contain.text', 'Temporal Coverage')
      cy.getByData(testIds.product_checkbox).eq(9).should('contain.text', '1 year')
      cy.getByData(testIds.product_checkbox).eq(10).should('contain.text', '5 year')
      cy.getByData(testIds.product_checkbox).eq(11).should('contain.text', '10 year').click()

      cy.getByData(testIds.product_details_checkbox).eq(4).should('contain.text', 'Data Formats')
      cy.getByData(testIds.product_checkbox).eq(12).should('contain.text', 'XML')
      cy.getByData(testIds.product_checkbox).eq(13).should('contain.text', 'CSV')
      cy.getByData(testIds.product_checkbox).eq(14).should('contain.text', 'NetCDF')
      cy.getByData(testIds.product_checkbox).eq(15).should('contain.text', 'HDF').click()
      ...
      cy.getByData(testIds.product_radio_text).eq(2).should('contain.text', 'Email')
      cy.getByData(testIds.product_radio_text).eq(3).should('contain.text', 'Rest API').click()

      cy.getByData(testIds.product_checkbox).eq(16).should('contain.text', 'Accept').click()
