...
describe('Search Page Tests', () => {
  ...
  context('When there are no search results', () => {
    ...
    beforeEach(() => {
      ...
      cy.performSearch(searchTerm, {
        fixture: '/ODR/searchPage/emptySearchResults.json'
      })
    })
  ...
  context('When there are search results', () => {
    ...
    it('should handle sort by price option in filter', () => {
      ...
      cy.getByData(testIds.searchpage_products).eq(0).should('contain.text', 'Mediation Services Four')
    })

    it('should handle filter by rating option in filter', () => {
      cy.getByData(testIds.searchpage_filterByRating).select(1)
      cy.getByData(testIds.searchpage_applyFilter).click()
      cy.getByData(testIds.searchpage_products).eq(0).should('contain.text', 'Mediation Services Two')
    })
    ...
    it('should handle reset applied filter', () => {
      ...
      cy.getByData(testIds.searchpage_products).eq(0).should('contain.text', 'Mediation Services One')
    })

    it('should navigate to product details page on item click', () => {
