...
describe('End to End test cases for Harmoni Aid app', () => {
  const deployed_url = 'https://harmoniaid-dev.becknprotocol.io/signIn'
  ...
  const product = 'Population disability data in Sylhet'
  const selectProductForCheckout = 'Population disability data in Sylhet'
  const filterByPriceProduct = 'Demographic exposure and vulnerability data in Sylhet'
  const filterByRatingProduct = 'Health related population exposure and vulnerability data in Sylhet'
  const filterReset = 'Health related population exposure and vulnerability data in Sylhet'
  const item_quantity = 'X 1'
  const item_price = '₹50.00'
  const totalPayment = '₹50.00'
  ...
  context('Product Details', () => {
    ...
    it('should validate and select all checkbox from every section ', () => {
      ...
      cy.getByData(testIds.product_checkbox).eq(0).should('contain.text', 'Vector').click()
      ...
      cy.getByData(testIds.product_checkbox).eq(6).should('contain.text', '80 m')
      cy.getByData(testIds.product_checkbox).eq(7).should('contain.text', '5 km').click()

      cy.getByData(testIds.product_details_checkbox).eq(3).should('contain.text', 'Data Formats')
      cy.getByData(testIds.product_checkbox).eq(8).should('contain.text', 'XML')
      cy.getByData(testIds.product_checkbox).eq(9).should('contain.text', 'CSV')
      cy.getByData(testIds.product_checkbox).eq(10).should('contain.text', 'HDF').click()

      cy.getByData(testIds.product_details_checkbox).eq(4).should('contain.text', 'Data Sharing Modes')
      ...
      cy.getByData(testIds.product_details_checkbox).eq(5).should('contain.text', 'Subscription Duration')
