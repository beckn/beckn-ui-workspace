...
import { statusResponse } from '../../../fixtures/Climate-resilience/SKY-ANALYTICS/orderHistory/statusResponse'
...
describe('OrderHistory Page Tests', () => {
  ...
  //     cy.intercept('GET', '**/api/orders?filters[category][id][$eq]=10&sort=updatedAt:desc', {
  //       fixture: 'Climate-resilience/SKY-ANALYTICS/orderHistory/emptyOrderResponse.json'
  //     }).as('getEmptyOrders')
  //   })
  ...
  context('Should render Order History Page if there is Order response Data ', () => {
    before(() => {
      ...
      cy.getByData(testIds.threeDots).click()
      cy.getByData(testIds.orderHistory_text_click).click()
      cy.intercept('GET', '**/api/orders?filters**', {
        fixture: 'Climate-resilience/SKY-ANALYTICS/orderHistory/orderResponse.json'
      }).as('orderResponse')
    })
